...
router.post('/search/productsByName', (req, res) => {
    const db = req.app.db;
    let name = req.body.name;
    // Get the item from the DB
    db.products.find({"productTitle" : {'$regex' : '.*' + name + '.*', '$options': 'i'}}, { productTitle: 1, productPrice: 1 }).toArray((err, product) => {
        if(err){
            console.error('Error');
            return res.status(400).json({message: 'Error updating cart. Please try again.'});
        }
...
router.post('/search/checkbyprice', (req, res) => {
    const db = req.app.db;
    let from = req.body.from ? parseInt(req.body.from) : '';
    let to = req.body.to ? parseInt(req.body.to) : '';

    if(from === '' || to === '') {
        return res.status(400).json({message: 'From and To value should not be empty'});
    }
    // Get the item from the DB
    db.products.find({productPrice: {$gte: from, $lte: to}}, { productTitle: 1, productPrice: 1 }).toArray((err, product) => {
        if(err){
            console.error('Error');
            return res.status(400).json({message: 'Error updating cart. Please try again.'});
        }
...
router.post('/getallproducts', (req, res) => {
    const db = req.app.db;
    // Get the item from the DB
    db.products.find({}, { productTitle: 1, productPrice: 1 }).toArray((err, product) => {
        if(err){
            console.error('Error');
            return res.status(400).json({message: 'Error updating cart. Please try again.'});
        }
